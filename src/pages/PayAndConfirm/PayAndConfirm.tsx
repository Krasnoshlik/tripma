...
export default function PayAndConfirm() {
  ...
  const [errors, setErrors] = useState<{ name?: string, number?: string, expiryDate?: string, cvv?: string }>({});
  ...
  return (
    <main className="max-w-7xl m-auto mt-[100px] mb-20 px-4 flex justify-between">
      {/*left section*/}
      <section className=" max-w-[768px] flex flex-col gap-8">
        ...
        <div className=" flex flex-col gap-2 max-w-[400px]">
          ...
          <div>
            <h3 className=" text-light-grey text-lg">{flightState.passenger.pFirstName} {flightState.passenger.pLastName}, {flightState.passenger.pEmail}, +{flightState.passenger.pKnowPhone}</h3>
          </div>
          <div>
          ...
          <div className=" flex flex-col gap-1">
            <div className=" flex justify-between text-light-grey"><p>Departing Flight</p><p>${flightState.flight.PickedFlight.price}</p></div>
            <div className=" flex justify-between text-light-grey"><p>Baggage fees</p><p>$0</p></div>
            <div className=" flex justify-between text-light-grey"><p>Seat upgrade (business)</p><p>$ 200</p></div>
            <div className=" flex justify-between text-light-grey"><p>Subtotal</p><p>$ {200 + flightState.flight.PickedFlight.price}</p></div>
            <div className=" flex justify-between text-light-grey"><p>Taxes (9.4%)</p><p>$ {((200 + flightState.flight.PickedFlight.price) * 0.094).toFixed()}</p></div>
          </div>
          <div className=" border-y py-2 mt-4 flex justify-between">
          <p>Subtotal</p><p>$ {200 + flightState.flight.PickedFlight.price + +((200 + flightState.flight.PickedFlight.price) * 0.094).toFixed()}</p>
          </div>
          </div>
        </div>
        ...
        <div className=" flex flex-col gap-2 max-w-[400px]">
          ...
          <h2 className=" text-sm text-light-grey">(hover card below to see CVV and click to change information)</h2>

          <div className="relative w-[300px] h-[188px] group perspective hover:cursor-pointer" onClick={() => setCardInfoModalOpen(true)}>
            <motion.div
            ...
            >
              ...
              >
                <div className="h-full flex flex-col justify-between items-start">
                  <img src={card.type} alt="card type" className="max-h-6" />
                  <div>
                    <h3>{card.name}</h3>
                    <h3>{card.number}</h3>
                  </div>
      ...
      <div className=" fixed top-0 left-0 w-screen h-screen bg-gray-500 bg-opacity-50 flex">
        <div className=" relative m-auto bg-white max-w-96 rounded-lg p-4 flex flex-col gap-4">
          <div className=" flex justify-between gap-10">
            <h3 className=" font-bold text-gray-500 text-2xl">Fill payment information</h3>
          ...
          <label htmlFor="cardName" className="flex flex-col gap-1">
            ...
            <input 
              type="text" 
              className={`border ${errors.name ? 'border-red-500' : 'border-light-grey'} rounded-sm outline-none pl-2`}
              value={cardName} 
          ...
          <label htmlFor="cardNumber" className="flex flex-col gap-1">
            ...
            <input 
              type="number" 
              className={`border ${errors.number ? 'border-red-500' : 'border-light-grey'} rounded-sm outline-none pl-2`}
              value={cardNumber} 
          ...
          <div className="flex gap-5">
            ...
            <label htmlFor="expiryDate" className="flex gap-2">
              ...
              <input 
                type="month" 
                className={`border ${errors.expiryDate ? 'border-red-500' : 'border-light-grey'} rounded-sm outline-none pl-2 max-h-[30px] max-w-[140px]`}
                value={expiryDate} 
            ...
            <label htmlFor="cvv" className="flex gap-2">
              ...
              <input 
                type="number" 
                className={`border ${errors.cvv ? 'border-red-500' : 'border-light-grey'} rounded-sm outline-none text-sm pl-2 max-w-[50px] max-h-[30px]`}
                value={cvv} 
          ...
          {/* Save button */}
          <button className=" border border-light-grey" onClick={handleSaveCardInfo}>Save info</button>
        </div>
      </div>
      }
    </main>
